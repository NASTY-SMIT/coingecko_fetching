...
class MyExchange(BaseExchange):
    """
        docs: https://docs.coingecko.com/v3.0.1/reference/endpoint-overview
        Takes all coins from the CoinGecko exchange and matches them to ETH.
    """
    ...
    def _convert_symbol_to_ccxt(self, symbols: str) -> Symbol:
        if isinstance(symbols, str):
            symbol = symbols.upper() + '/ETH'
            return symbol

    def normalize_data(self, data: dict) -> dict[Symbol, TickerInfo]:
        normalized_data = {}
        for item in data:
            symbol = self._convert_symbol_to_ccxt(item['symbol'])
            ticker_info = TickerInfo(
                last=item['current_price'],
                baseVolume=item['total_volume'],
                quoteVolume=item['market_cap']
            )
            normalized_data[symbol] = ticker_info
        return normalized_data

    async def fetch_tickers(self) -> dict[Symbol, TickerInfo]:
        data = await self.fetch_data(
            self.base_url + 'coins/markets?vs_currency=eth')
